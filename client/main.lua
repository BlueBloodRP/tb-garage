        ...
        for k, v in pairs(Config.Garages) do
            ...
            if dist < 1.5 then
                if not IsPedInAnyVehicle(ped, false) then
                    TBCore.Functions.DrawText3D(Config.Garages[k].x, Config.Garages[k].y, Config.Garages[k].z + 0.5, '[~g~E~w~] To get a vehicle')
...
RegisterNUICallback('spawn', function(vehicle)
    getOutVehicle(vehicle)
end)
...
Citizen.CreateThread(function()
	...
	while true do
        if(menu) then
            ...
            TriggerEvent("GUI:Option", "Close", function(cb)
                if(cb) then
                    menu = not menu
                end
            end)
